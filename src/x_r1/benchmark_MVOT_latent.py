...
def intermediate_majority_voting(model, tokenizer, selected_context, intermediate_results, layer, batch_size=2):
    ...
    with torch.inference_mode():
        ...
        for i in range(0, len(full_context_list), batch_size):
            # also return the offset mapping to find the position of <answer> or <final> tag
    ...
    for prompt, gold_answer, gold_process in zip (prompts, answers, processes):
        ...
        for i in range(args.max_steps + 1):
            ...
            for output in outputs:
                if i == args.max_steps:
                    if output.outputs[0].text.find("<final>") != -1 and output.outputs[0].text.find("</final>") != -1:
                        intermediate_results.append(output.outputs[0].text)
                elif output.outputs[0].text.find("<answer>") != -1:
                    intermediate_results.append(output.outputs[0].text + "</answer>")
                elif output.outputs[0].text.find("<final>") != -1:
                    intermediate_results.append(output.outputs[0].text + "</final>")
