    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
                ...
                if self.quick_eval_dataset is not None and (self.state.global_step/self.num_iterations) % self.args.eval_steps == 0 and self.run_quick_eval:
                    ...
                    for i, reward_func in enumerate(self.reward_funcs):
                        if isinstance(reward_func, nn.Module):
                            ...
                            output_reward_func, steps_final_pos = reward_func(completions=quick_eval_completions, reward_type = self.reward_type, silence = True, **reward_kwargs)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                output_reward_func, steps_final_pos = reward_func(prompts=prompts, completions=completions, reward_type = self.reward_type, **reward_kwargs)
